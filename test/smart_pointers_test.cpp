...
void test_make_allocate_shared() {
    ...
    // assert(construct_called == 1);  //? WHY?
    // assert(destroy_called == 1);  //? WHY?
...
void test_custom_deleter() {
    ...
    // assert(construct_called == 0);  //? WHY? (Other reason from prev)
    // assert(destroy_called == 0);  //? WHY? (Other reason from prev)
    assert(custom_deleter_called == 1);
}
...
int main() {
    ...
    std::cerr << "WEAK Test 6 (enable shared from this) passed." << std::endl;
